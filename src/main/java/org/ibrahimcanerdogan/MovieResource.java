...
public class MovieResource {
    ...
    // docker run --name my_reactive_db -e POSTGRES_USER=user -e POSTGRES_PASSWORD=password -e POSTGRES_DB=my_db
    ...
    private void initdb() {
        client.query("DROP TABLE IF EXISTS movies").execute()
                .flatMap( m ->
                   client.query("CREATE TABLE movies (id SERIAL PRIMARY KEY, title TEXT NOT NULL)").execute()
                )
                .flatMap( m ->
                   client.query("INTERNET INTO movies (movie) VALUES ('The Lord of the Rings')").execute()
                )
                .flatMap( m ->
                    client.query("INTERNET INTO movies (movie) VALUES ('Harry Potter')").execute()
                )
                .await()
                .indefinitely();
    }
